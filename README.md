...
**The nav links to four pages that do not exist yet** (`physical-rooms.html` and `operations-calendar.html` are both built now). This is intentional, following the same "don't build every screen from one prompt" sequencing established earlier in the project. Expect follow-up prompts to build: `guests.html`, `reservation-reports.html`, `inventory-reports.html`, `payment-reports.html`, `hotels.html`. When you build them, just create the file at that exact filename and the nav + breadcrumb linking will work automatically (add the new labels to `CRUMB_LINKS` too).
...
Interaction pattern only loosely modeled on Mews-style hotel operations calendars (no branding, copy, or pixel-for-pixel layout copied) — a split view: a horizontal timeline grid on the left/main area, and a right-side drawer for whichever reservation, unassigned unit, or room block is selected. The main grid stays visible and scrolled-in-place behind the drawer (it's an overlay, not a route change) so staff never lose date/room context.
...
- **Conflict detection is real, not styled-in.** `barsForRoom()` flags any two bars on the same physical room whose `[start,end)` ranges overlap — this is the same half-open-interval check used everywhere else in the engine. The seeded `blk-3` (Management Hold) vs. `asn-5` (RES-10247's Held assignment) on `fam-402` is a live, out-of-the-box demonstration of the `.opc-bar.conflict` styling (thick red outline + hazard stripes + a warning icon), not a special case.
...
- **Unassigned Reservations lane** (`unassignedUnits()`) is a set of one-row-per-missing-unit "virtual" rows pinned above the room-type groups, only rendered when non-empty. Sorted soonest-arrival-first; `urgencySeverity()` buckets by `PG.nightsBetween(TODAY, checkIn)` into danger (≤1 day) / warning (≤3 days) / neutral. The seeded `RES-10248` (Layla Nasser, no `roomAssignment` at all) demonstrates this out of the box.
- **Change Room / Assign Room reuse `PG.renderChangeRoomDrawer()`** (§4.7) — this page never reimplements a room picker. Unassign Room is gated on `CAN_MANAGE` and requires a reason via a dedicated modal, exactly like `reservation-detail.html`'s. Cancel Reservation is an *inline* reveal inside the Reservation drawer (reason + Confirm/Keep), not the full before/after-inventory modal `reservation-detail.html` uses — a deliberate scope reduction to avoid duplicating that whole flow in a second file; "Open/Edit Reservation" and "Open Payment" all link to `reservation-detail.html` rather than reimplementing those surfaces here. There is no Check In / Check Out action anywhere, per spec.
...
- **Fixing this file surfaced a real bug in `reservation-detail.html`'s cancel flow**: cancelling a reservation set `status='Cancelled'` but never touched `state.roomAssignments`, so a cancelled reservation's room stayed "Reserved" forever from the physical-assignment layer's point of view. Fixed in the same change — `doCancel()` now also marks every non-Cancelled assignment for that reservation as `Cancelled`. Any future cancellation path (there are now two — `reservation-detail.html` and this page's inline cancel) must do the same.
