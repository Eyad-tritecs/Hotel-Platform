...
- **Hotel setup** — tenant profile, room types (CRUD), rate plans, an editable per-date rate calendar (cell-level edit drawer + a range/day-of-week Bulk Rate Change modal), and a real, configurable Taxes & Fees engine (percentage/fixed, tax/fee, default applicability, optional effective dates) feeding pricing everywhere.
...
```
{
  ...
  rates, bedConfigs, mealPlans, taxesFees,
...
- **`rates`** — `{ [roomTypeId]: { [date]: price } }`, a per-date calendar used by `PG.rateFor()`. This is the actual pricing engine — **not** the same thing as `ratePlans` below, which is an admin-facing display concept. Edited via `rates.html`'s rate calendar (§8.5).
...
| `rates.html` | **Rate calendar** (room types × dates, the actual `state.rates` pricing engine) plus the Rate Plan list | See §8.5. Sticky room-type column + sticky date header; each cell shows the effective rate, an availability dot, and a Stop Sell flag. Click a cell for a focused **Edit Rate drawer**; **Bulk Rate Change** is a separate range-based modal (room types, dates, days of week, set/±amount, preview, confirm). Rate Plan list below is unchanged. |
...
### 8.5 Availability & Inventory, Rates, and Taxes & Fees — key behaviors to preserve
...
- **The rate calendar edits `state.rates` directly — never a spreadsheet-style unrestricted inline grid.** Every cell click opens a single-date **Edit Rate drawer** (current effective rate prefilled, an explicit Save Rate / Reset to Base Rate action) — nothing is written by typing into the grid itself. **Bulk Rate Change** is a separate modal: room types (checkboxes), a date range, days-of-week chips, a Set/Increase/Decrease mode with one amount, a live preview (affected-date chips + a total count), a `confirm()` gate, and one summarizing `audit` entry per bulk action — never one entry per date. A bulk change that would take any date's rate to $0 or below skips just that entry (reported in the toast/audit) rather than writing an invalid rate.
...
- Every rate change, bulk rate change, inventory adjustment, Stop Sell/Reopen, and tax/fee change writes one `audit` entry (`'Rate Changed'`, `'Inventory Adjusted'`, `'Stop Sell Applied'`/`'Inventory Reopened'`, `'Tax/Fee Changed'`) — consistent with every other mutation in this codebase.
